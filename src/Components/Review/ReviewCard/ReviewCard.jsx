...
import { addOneReaction, changeReaction, removeReaction } from "../../../Slices/reviews.ts";
    ...
    const handleReaction = async (user_liked, user_disliked, review_id, user_id, reaction, access_token) => { // reaction: true = like, false = dislike    
        try {
            if ((user_liked && !reaction) || (user_disliked && reaction)) { // two truth cases: user previously liked and wants to dislike OR user previously disliked and wants to like
                ...
                const updatedReaction = await updateReaction({ id: foundReaction.id, reaction }, access_token); // update that specific reaction to the opposite react
                dispatch(changeReaction({ reaction: updatedReaction }))
                return;
            }
            ...
            if ((user_liked && reaction) || (user_disliked && !reaction)) { // undo the liking and disling of a post
                ...
                dispatch(removeReaction({ reaction: { id: foundReaction.id, review_id } }))
                return;
            }

            const createdReaction = await createReaction(review_id, user_id, reaction, access_token); // if user hasn't liked or dislike a given review, create a reaction
            dispatch(addOneReaction({ reaction: createdReaction }))
        } catch (error) {
            console.error(error);
        }
    }
